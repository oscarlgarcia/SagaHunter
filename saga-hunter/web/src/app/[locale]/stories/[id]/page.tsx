...
import {
  ...
  BookText, MapPin, Users, Layout, FileText, Send,
} from "lucide-react";
...
export default function StoryDetailPage() {
  ...
  const runStep = async (step: string) => {
    setRunningStep(step);
    try {
      await fetch(`/api/stories/${id}/step`, {
  ...
  const runAll = async () => {
    setRunningAll(true);
    try {
      await fetch(`/api/stories/${id}/run`, { method: "POST" });
      await load();
        ...
        {STEP_ORDER.map((step, i) => {
          ...
          return (
            <div key={step} className="flex items-center gap-2 flex-1">
              ...
              {i < STEP_ORDER.length - 1 && <div className="flex-1 h-px bg-gray-200" />}
            </div>
          );
        })}
      </div>
