...
class AngleFinder(BaseAgent):
    ...
    def execute(self) -> AgentResult:
        seeds = execute(
            "SELECT s.id, s.raw_text, s.title, s.language FROM seeds s LEFT JOIN enrichments e ON e.seed_id = s.id AND e.agent_name = %s WHERE e.id IS NULL ORDER BY s.discovered_at DESC",
            (self.name,),
            fetch=True,
        )
        enriched = 0
